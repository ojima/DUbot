class Sutran:
    ...
    def __init__(self):
        board = []
        for i in range(9):
            board.append([None for j in range (9)])
        self._board = board
    ...
    def overruns(self):
        ...
        for i in range(1, 8):
            for j in range(1, 8):
                ...
                if(unit != None):
                    if(self._board[i+1][j+1].owner() != unit.owner() and self._board[i-1][j+1].owner() != unit.owner() and self._board[i+1][j-1].owner() != unit.owner() and self._board[i-1][j-1].owner() != unit.owner()):
...
class Player:
    ...
    def place(self, type, location):
        if(self.canPlace(type) and self._game.board()[location[0]][location[1]] == None):
            for unit in self._reserve:
                if(unit.type() == type):
                    self._reserve.remove(unit)
